...
TEST_CASE("Full test of Identity Packets", "[Identity Data][Behavioural]") {
  ...
  CHECK(encrypted_master_data1 == encrypted_master_data2);
  CHECK(encrypted_surrogate_data1 == encrypted_surrogate_data2);
  ...
  CHECK(tmid_name1 == tmid_name2);
  CHECK(stmid_name1 == stmid_name2);
  CHECK(tmid1.encrypted_session() == tmid2.encrypted_session());
  CHECK(stmid1.encrypted_session() == stmid2.encrypted_session());
  ...
  CHECK(mid_value1 == mid_value2);
  CHECK(smid_value1 == smid_value2);
  ...
  CHECK(mid1.name() == mid2.name());
  CHECK(smid1.name() == smid2.name());
  CHECK(mid1.encrypted_tmid_name() == mid2.encrypted_tmid_name());
  CHECK(smid1.encrypted_tmid_name() == smid2.encrypted_tmid_name());
  ...
  CHECK(decrypted_master_data1 == decrypted_master_data2);
  CHECK(kMasterData == decrypted_master_data2);
  CHECK(decrypted_surrogate_data1 == decrypted_surrogate_data2);
  CHECK(kSurrogateData == decrypted_surrogate_data2);
  ...
  CHECK(decrypted_mid_value1 == decrypted_mid_value2);
  CHECK(tmid_name1 == decrypted_mid_value2);
  CHECK(decrypted_smid_value1 == decrypted_smid_value2);
  CHECK(stmid_name1 == decrypted_smid_value2);
...
TEST_CASE("Change details of Identity Packets", "[Identity Data][Behavioural]") {
  ...
  CHECK(dec2 == next_master2);
  CHECK(dec1 == next_master2);
}
