 ...
 protected:
  ...
  bool VerifySaveSession() {
    ...
    if (passport_.IdentityPacketName(kTmid, true) ==
      ...
      NonEmptyString hash_pin(sha512_hash(NonEmptyString(boost::lexical_cast<std::string>(pin_)))),
  ...
  bool VerifyChangeDetails(const NonEmptyString& new_keyword, uint32_t new_pin) {
    ...
    NonEmptyString new_pin_hash(
        sha512_hash(NonEmptyString(boost::lexical_cast<std::string>(new_pin))));
    ...
    NonEmptyString pin_hash(sha512_hash(NonEmptyString(boost::lexical_cast<std::string>(pin_)))),
