...
#ifndef MAIDSAFE_PASSPORT_DETAIL_SAFE_ALLOCATORS_H_
...
// small objects that span up to a few pages, mostly smaller than a page. To support large 
// allocations, something like an interval tree would be the preferred data structure.
...
#ifdef MAIDSAFE_WIN32
    if (VirtualLock(const_cast<void*>(address), length) != 0)
      return true;
    else
      return false;
#else
...
#ifdef MAIDSAFE_WIN32
    if (VirtualUnlock(const_cast<void*>(address), length) != 0)
      return true;
    else
      return false;
#else
...
class LockedPageManager
  : public LockedPageManagerBase<MemoryPageLocker> {
public:
...
struct safe_allocator
  ...
  template<typename U>
  safe_allocator(const safe_allocator<U>& a) : base(a) {}
  ~safe_allocator() {}
...
struct zero_after_free_allocator
  ...
  template <typename U>
  zero_after_free_allocator(const zero_after_free_allocator<U>& a) : base(a) {}
  ~zero_after_free_allocator() {}
